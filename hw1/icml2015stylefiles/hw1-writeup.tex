...
% For the blog feedback data there is only one parameter to choose during model selection, namely $\lambda$ since the feature set $\Phi$ has already been specified for us. The objective function that is being minimized during ridge regression is
...
% %
%
We ran ridge regression with a variety of $\lambda$ values and saw that $\lambda \in [0.01,2]$ provided the best performance. Thus we computed ridge regression for a grid of $\lambda$ values in $[0.01,2]$. The results are shown in Figure \ref{blog-model-selection}.
%
%
%
\begin{figure}[h]
