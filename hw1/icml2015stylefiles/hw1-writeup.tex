...
Problem 4.1, ``Outlier and LAD,'' was chosen.  We implemented minimization of the sum of absolute values of the errors (SAE) via gradient descent, also termed LAD (least absolute deviations) regression.  In contrast to OLS, there is no matrix manipulation that can lead to a closed form solution for LAD.  We instead must use a different approach -- here, we descend the SAE objective function via gradient descent.  Since the SAE is not differentiable at $| y - h(x)|=0$, we deferred to letting our optimizer use numerical gradients.
...
We repeat the model selection experiments that we performed in Problem 3, but for SAE ($|y-h(x)|$) rather than SSE ($(y-h(x))^2$). We search over $(M,\lambda)$ for the best fit measured by the SAE on the validation dataset, $SAE_v(M,\lambda)$, produced from the regression weights $w(M,\lambda)$ found by LAD regression on the test set.  Again, the range $\lambda \in [0,10]$ was used. Table \ref{model-selection-SAE} shows the results.

This process for LAD model selection results in selecting a linear fit, $M=1$, with a small regularization term $\lambda=0.85$.  

As was examined in Problem 3
