...
The parametric form of the policy (for commanding the steering rate $u$) was chosen to take the following form: 
%
%
\begin{equation}
u = (\sigma(\theta^T \phi_B(X)) - \frac{1}{2}) * u_{max} 
\end{equation}
%
%
%where $\sigma$ is the logistic sigmoid function, $\phi_B(x_i) = \frac{1}{x_i^2}$ as for the Braitenberg vehicle.  First we note that the above is deterministic, so that the stochasticity of the policy is on the parameters rather than the output.  
