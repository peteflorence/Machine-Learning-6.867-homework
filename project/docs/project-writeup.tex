...
\scalebox{0.7}{
  \begin{tabular}{l|c|c|l|c|c|c|c|c}
	\textbf{Car and sensor parameters}\\
	Car velocity, $v$ & 16 m/s \\ 
	Maximum turning rate, $u_{max}$, 4 \\
...
As a baseline against which to compare our learned controllers, we used a simple hand-designed controller inspired by the controllers of Valentino Braitenberg.  The simplest controller we tried used simply counted the number of non-max sensor returns on the left ($n_{left} = \sum_i [x_i < d_{max}]$ for $i = 1, ..., N/2)$ and compared to the number of non-max sensor returns on the right ($n_{right} = \sum_i [x_i < d_{max}]$ for $i = N/2 + 1, ..., N)$), and then selected the action to turn towards the direction $min(n_{left}, n_{right})$.  If no obstacles were seen ($n_{left} = n_{right} = 0$), then the controller would select to go straight.  Thus the action space for this simple controller is $A = \{-u_{max}, 0, u_{max} \}$.  

Then describe controller based on $\phi$...
