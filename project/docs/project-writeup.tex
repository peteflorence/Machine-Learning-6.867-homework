...
% \icmltitlerunning{Reinforcement Learning for Autonomous Driving Obstacle Avoidance using LIDAR}

\icmltitlerunning{RL for Obstacle Avoidance}
% \icmltitle{Reinforcement Learning for Autonomous Driving Obstacle Avoidance using LIDAR}
...
\begin{center}
  ...
  \captionof{figure}{Screen capture from simulation setup: a car in a circular obstacle field with an array of point LIDAR measurements.}
  \label{fig:test}
\end{center}
...
\captionof{table}{Simulation parameters used for the experiments, unless otherwise noted.}
\end{center}
...
% In our specific setup this will not be a state in the true state of the word (sinc ethe true state is only partially observable), but rather will just be our observation (in this case the LIDAR measurement).
%
...
\begin{algorithmic}
   ...
   \FOR{each step of episode}
       ...
       \STATE $A' \leftarrow$ policy derived from Q-values at state $S'$ (using $\epsilon$-greedy)
       \STATE $\delta \leftarrow R + \gamma Q(S', A', \mathbf{w}) - Q(S,A, \mathbf{w})$
       \STATE $\mathbf{w}_{old} \leftarrow \mathbf{w}$
       \STATE $\mathbf{w} \leftarrow \mathbf{w} + \alpha \delta \mathbf{z}$
       \STATE $\mathbf{z} \leftarrow \gamma \lambda z + \nabla_{\mathbf{w}} Q(S,A,\mathbf{w}_{old})$
       \STATE $S \leftarrow S', A \leftarrow A'$
   \ENDFOR}
   \UNTIL{end of episode}
...
%
\begin{algorithm}[H]
   \caption{Variant of Episodic REINFORCE inspired by Roberts et al.}
   \label{alg:REINFORCE}
\begin{algorithmic}
   % \STATE {\bfseries Input:} sensor data $x_i$, size $N$
   % \STATE {\bfseries Output:} u from $\mathcal{A} =  \{-u_{max}, 0, u_{max}\}$
   \STATE Initialize policy parameterization $\theta_i$ = $\theta_0$
   \REPEAT
   
   \STATE compute $R_{total}$ for $\infty$-horizon roll-out with $u(\theta_i)$
   \STATE randomly select index $j$ for one of $\theta_i$ in $\theta_{left}$
   \STATE $\theta_{p_i} =$ zero vector, same length as $\theta$
   \STATE $\theta_{p_i}[j] = \mathcal{N}(0,\sigma_p^2)$   
   
   \STATE compute $R'_{total}$ for $\infty$-horizon roll-out with $u(\theta_i +\theta_{p_i})$
   
   \STATE update policy: $$\theta_{i+1} = \theta_i + \eta (R_{total} - R'_{total})\theta_{p_i}$$
...
\caption{Comparison of performance across 10 runs of discrete SARSA using parameters in Table \ref{table:defaultParameters}. The duration is in simulator ticks, there are 20 ticks/second.}
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{figures/policySearchFigs/zeroedParams/eta5e_neg2/overtime.png}
\caption{Performance (run duration and discounted reward) over time of the policy search controller (Episodic REINFORCE), initialized with all zero weights.  Parameters for the gradient descent update were $\eta = 5e-2$ and $\sigma_p^2 = 4.0$.  }
\label{zeroedParamsOverTime}
\end{figure}

\begin{figure}
\centering
\includegraphics[scale=0.5]{figures/policySearchFigs/zeroedParams/eta5e_neg2/finalweights.png}
\caption{Initial (all zero) and final weights $\theta_{left}$ of the policy search controller (Episodic REINFORCE), for the same trained controller as in the figure above.  }
\label{zeroFinalWeights}
\end{figure}

\begin{figure}
\centering
\includegraphics[scale=0.5]{figures/policySearchFigs/okayedParams/cov6/overtime.png}
\caption{Performance (run duration and discounted reward) over time of the policy search controller (Episodic REINFORCE), initialized with sensible but imperfect weights (as plotted in the figure below).  Parameters for the gradient descent update were $\eta = 2e-2$ and $\sigma_p^2 = 6.0$.  }
\label{okayParamsOverTime}
\end{figure}

\begin{figure}
\centering
\includegraphics[scale=0.5]{figures/policySearchFigs/okayedParams/cov6/weights.png}
\caption{Initial and final weights $\theta_{left}$ of the policy search controller (Episodic REINFORCE), for the same trained controller as in the figure above.  }
\label{okayParamsFinalWeights}
\end{figure}
